package com.mzl.authcssodemo.uitl;
...
public class SignUtils {
     ...
     * @return
     */
    public static Boolean checkSign(HttpServletRequest request,String sign, String username){
        Boolean flag= false;
        //检查sigin是否过期（是否正确）
//        Enumeration<?> pNames =  request.getParameterNames();   //获取当前请求的所有参数(请求参数方式)
        ...
        Map<String, String> params = new HashMap<String, String>();   //存储进行sign加密所需要的参数（token、timestamp和username）
...
//                String pValue = (String)request.getParameter(pName);  //获取请求参数的值（请求参数方式）
    ...
    /**
     * 得到签名(签名=时间戳+用户的唯一token+密匙+用户名(用户id) 的四者进行MD5加密生成的)
